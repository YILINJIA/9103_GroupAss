...
//set the timer to 10 seconds for the animation
...
let colors = {
  ...
  black:  {r: 0,   g: 0,   b: 0},
  darkBlue: {r: 0, g: 0, b: 102},
  lightBlue: {r: 135, g: 206, b: 235},
  white: {r: 255, g: 255, b: 255},
  lightYellow: {r: 255, g: 255, b: 51}
}

let backgroundColors = [colors.lightYellow, colors.white, colors.lightBlue, colors.darkBlue];
...
function setup() {
  ...
  let bgColor = backgroundColors[currentColorIndex];
...
function draw() {
  ...
  if (millis() - lastTransitionTime >= transitionDuration) {
    currentColorIndex++;
  ...
  let bgColorStart = backgroundColors[currentColorIndex];
  let bgColorEnd = backgroundColors[(currentColorIndex + 1) % backgroundColors.length];
  let lerpAmount = (millis() - lastTransitionTime) / transitionDuration;
  let r = lerp(bgColorStart.r, bgColorEnd.r, lerpAmount);
  let g = lerp(bgColorStart.g, bgColorEnd.g, lerpAmount);
  let b = lerp(bgColorStart.b, bgColorEnd.b, lerpAmount);
  background(r,g,b);
  ...
  for (let square of verticalSquares) {
    square.y += square.speed * speedMultiplier;
    if (square.y > height) {
  ...
  for (let square of horizontalSquares) {
    square.x += square.speed * speedMultiplier;
    if (square.x > width) {
  ...
  if (currentColorIndex === 2) { // Check if the color is going to be dark blue
      // Change the color of the squares to black
      squares.map((square) => {
        if (square.color === colors.grey) {
          square.color = colors.yellow
        }  
      })
      if (speedMultiplier >0) {
        speedMultiplier -= 0.002; // Decrease the speed gradually
      }
  } else if (currentColorIndex === 3) { // Check if the color is going to be yellow
      // Change the color of the squares to black
      squares.map((square) => {
        if (square.color === colors.yellow) {
          square.color = colors.darkBlue
        }  
      })
      if (speedMultiplier < 1) {
        speedMultiplier += 0.002; // Increase the speed gradually
      }  
  } else {
    squares.map((square) => {
      if (square.color === colors.darkBlue) {
        square.color = random([colors.yellow,colors.grey])
      }  
    })
  }
